...
namespace myslam 
{
    Frontend::Frontend() 
    {
        //在config中可以看到配置，这里是创建一个GFTT检测器，
        //150：表示检测到的最大角点数量。这个值决定了检测器在给定的图像中将要找出多少个特征点。
        //0.01：表示检测到的角点的最小响应值。这个值决定了检测器在寻找角点时，响应值低于这个值的特征点将会被忽略。
        //20：这是角点的最小距离，表示在寻找特征点时，相邻两个特征点之间的最小距离。这个值越大，特征点之间的距离越远，从而可能减少相互重叠的特征点。
...
//先来StereoInit
    bool Frontend::StereoInit() {
        ...
        if (num_coor_features < num_features_init_) {//对应数目不足，无法初始化，也就是初始化必须满足能找到50及其以上的个匹配的点对
            return false;
        }
