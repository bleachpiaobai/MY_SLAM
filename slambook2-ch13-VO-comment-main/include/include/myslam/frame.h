 ...
 */
    ...
    struct Frame {
    public:
        EIGEN_MAKE_ALIGNED_OPERATOR_NEW;// Eigen 库（一个用于线性代数运算的 C++ 模板库）中的一个宏定义，作用就是为使用了它的类重新定义 new 和 delete 操作符，使得通过这些操作符分配和释放的内存能够按照 Eigen 要求的方式进行对齐，进而保证涉及这些自定义类型对象的各种运算（例如矩阵乘法、向量运算等）能正确、高效地利用硬件特性来执行
